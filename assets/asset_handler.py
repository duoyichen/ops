...
def log(log_type, msg=None, asset=None, new_asset=None, request=None):
    ...
    if log_type == "upline":
        event.name = "%s <%s> ：  上线" % (asset.name, asset.server_asset.sn)
        event.asset = asset
        event.detail = "资产成功上线！"
        ...
        event.name = "%s <%s> ：  数据更新！" % (asset.asset_type, asset.server_asset.sn)
        event.asset = asset
        event.detail = "更新成功！"
    elif log_type == "update_failed":
        event.name = "%s <%s> ：  更新失败" % (asset.asset_type, asset.server_asset.sn)
        event.asset = asset
        event.detail = "更新失败！\n%s" % msg
    # 更多日志类型.....
    event.save()
...
class ApproveAsset:
    ...
    def _server_device_upline(self):
    ...
    def _create_asset(self):
        ...
        asset = models.Asset.objects.create(asset_type=self.new_asset.asset_type,
                                            asset_code=self.new_asset.asset_code,
                                            manage_ip=self.new_asset.manage_ip,
                                            idc=self.new_asset.idc,
    ...
    def _create_server(self, asset):
        ...
        :return:
        """
        models.Server_Asset.objects.create(asset=asset,
...
class UpdateAsset:
    ...
    def _server_device_update(self):
        try:
